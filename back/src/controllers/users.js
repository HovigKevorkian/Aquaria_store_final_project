  ...
  const getUser = async id => {
    ...
    const deleteUsers = async (id)  => {
      ...
      const createUsers = async (props) => {
          const {first_name, last_name, email, phone_number, created_at, password , country, city , postal_code, street, unit} = props;
        try {
            if ( !props || !first_name || !last_name || !email || !password || !phone_number || !country || !postal_code || !city || !street || ! unit) {
      ...
      const updateUsers = async (id,  props) => {
        const {first_name, last_name, email, phone_number, created_at, password , country, city , postal_code, street, unit} = props;
          try {
        if (!id ||  (!id || (!first_name || !last_name || !email || !password || !phone_number || !country || !postal_code || !city || !street || ! unit)) ||
         (!first_name || !last_name || !email || !password || !phone_number || !country || !postal_code || !city || !street || ! unit)) {
          throw new Error("you must provide an id and/or one of the inputs");
        }
        
          const stmt = `UPDATE users SET first_name=("${first_name}"), last_name=("${last_name}"), email=("${email}"), phone_number=(${phone_number}), created_at=("${created_at}"), password=("${password}"), , country=("${country}"),city=("${city}"), postal_code=(${postal_code}), street=("${street}"), unit=("${unit}") WHERE user_id=(${id})`;
